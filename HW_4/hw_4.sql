...
UPDATE messages SET 
	to_user_id = FLOOR((rand()*100)),
    from_user_id = FLOOR((rand()*20));
UPDATE messages SET to_user_id = 100 WHERE to_user_id = 0;
...
UPDATE media SET 
    ...
    user_id = FLOOR(RAND()*100);
-- убераем несуществующие id юзеров равные 0
UPDATE media SET user_id = 100 WHERE user_id = 0;
